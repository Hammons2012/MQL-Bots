...
input int InpTDIMiddleandVWAPandHAMagicNumber = 11111;                     // Magic number for trades opened by TDI middle, VWAP, and HA strategy
input int InpVWAPandHAMagicNumber = 22222;                                 // Magic number for trades opened by VWAP and HA strategy
input int InpTDICrossoverMiddleMagicNumber = 33333;                        // Magic number for trades opened by TDI crossover and middle strategy
input int InpTDINoneandHAMagicNumber = 44444;                              // Magic Number for trades opened by TDI and HA strategy
input int InpTDICrossoverMagicNumber = 55555;                              // Magic Number for trades opened by TDI Crossover
...
input bool InpTDIMiddleandVWAPandHAStrategy = false;                       // Open trades based on TDI, VWAP middle, and HA indicators
input bool InpHAandVWAPStrategy = false;                                   // Open trades based on HA and VWAP indicators
input bool InpTDICrossoverMiddleStrategy = false;                          // Open trades based on TDI crossover and middle indicators
input bool InpTDINoneandHAStrategy = false;                                // Open trades based on TDI and HA indicators
input bool InpTDICrossoverStrategy = false;                                // Open trades based on TDI Crossover indicator
...
void OnTick()
{
  ...
  bool IsTDIMiddleandVWAPandHATradeOpen = IsTradeOpen(InpTDIMiddleandVWAPandHAMagicNumber, InpAllowBothTypePositions);
  bool IsVWAPandHATradeOpen = IsTradeOpen(InpVWAPandHAMagicNumber, InpAllowBothTypePositions);
  bool IsTDICrossoverMiddleTradeOpen = IsTradeOpen(InpTDICrossoverMiddleMagicNumber, InpAllowBothTypePositions);
  bool IsTDINoneandHATradeOpen = IsTradeOpen(InpTDINoneandHAMagicNumber, InpAllowBothTypePositions);
  bool IsTDICrossoverTradeOpen = IsTradeOpen(InpTDICrossoverMagicNumber, InpAllowBothTypePositions);
  ...
  if (Time[0] > LastBarTime)
  {
    ...
    // Trading logic
    // If strategy is allowed, there are no open trades, there is no open wick, the bar is bullish, RSI Price is higher than BB Middle, RSI Signal higher than BB Middle, RSI price is lower than RSI Signal, HA Open higher than VWAP, and HA open lower than VWAP positive deviation
    if (InpTDIMiddleandVWAPandHAStrategy && !IsTDIMiddleandVWAPandHATradeOpen && HAOpenCurrent == HALowCurrent && HAHighCurrent > HALowCurrent && TDIRSIPriceCurrent > TDIBBandMiddleCurrent && TDIRSISignalCurrent > TDIBBandMiddleCurrent && TDIRSIPriceCurrent > TDIRSISignalCurrent && HAOpenCurrent > VWAPCurrent && HAOpenCurrent < VWAPDeviationPositiveCurrent)
    {
      OpenBuyTrade(EntryPrice, StopLoss, TakeProfit, RiskAmount, InpStopLoss, InpRewardPercentage, InpTDIMiddleandVWAPandHAMagicNumber);
    }
  
    // If strategy is allowed, there are no open trades, there is no open wick, the bar is bearish, RSI Price is lower than BB Middle, RSI Signal is lower than BBand Middle, RSI price is lower than RSI Signal, HA Open lower than VWAP, and HA open higher than VWAP negative deviation
    if (InpTDIMiddleandVWAPandHAStrategy && !IsTDIMiddleandVWAPandHATradeOpen && HAOpenCurrent == HALowCurrent && HAHighCurrent < HALowCurrent && TDIRSIPriceCurrent < TDIBBandMiddleCurrent && TDIRSISignalCurrent < TDIBBandMiddleCurrent && TDIRSIPriceCurrent < TDIRSISignalCurrent && HAOpenCurrent < VWAPCurrent && HAOpenCurrent > VWAPDeviationNegativeCurrent)
    {
      OpenSellTrade(EntryPrice, StopLoss, TakeProfit, RiskAmount, InpStopLoss, InpRewardPercentage, InpTDIMiddleandVWAPandHAMagicNumber);
    }
  
    // If strategy is allowed, there are no open trades, there is no open wick, the open is higher than the daily VWAP, the bar is bullish, and the open is lower than the deviation
    if (InpHAandVWAPStrategy && !IsVWAPandHATradeOpen && HAOpenCurrent == HALowCurrent && HAOpenCurrent > VWAPCurrent && HAHighCurrent > HALowCurrent && HAOpenCurrent < VWAPDeviationPositiveCurrent)
    {
      OpenBuyTrade(EntryPrice, StopLoss, TakeProfit, RiskAmount, InpStopLoss, InpRewardPercentage, InpVWAPandHAMagicNumber);
    }

    // If strategy is allowed, there are no open trades, there is no open wick, the open is lower than the daily VWAP, the bar is bearish, and the open is higher than the deviation
    if (InpHAandVWAPStrategy && !IsVWAPandHATradeOpen && HAOpenCurrent == HALowCurrent && HAOpenCurrent < VWAPCurrent && HAHighCurrent < HALowCurrent && HAOpenCurrent > VWAPDeviationNegativeCurrent)
    {
      OpenSellTrade(EntryPrice, StopLoss, TakeProfit, RiskAmount, InpStopLoss, InpRewardPercentage, InpVWAPandHAMagicNumber);
    }
  
    // If strategy is allowed, there are no open trades, RSI Price is higher than BB Middle, RSI Signal higher than BB Middle, and there has been a buy crossover
    if (InpTDICrossoverMiddleStrategy && !IsTDICrossoverMiddleTradeOpen && TDIRSIPriceCurrent > TDIBBandMiddleCurrent && TDIRSISignalCurrent > TDIBBandMiddleCurrent && TDIBuyCrossover)
    {
      OpenBuyTrade(EntryPrice, StopLoss, TakeProfit, RiskAmount, InpStopLoss, InpRewardPercentage, InpTDICrossoverMiddleMagicNumber);
    }
  
    // If strategy is allowed, there are no open trades, RSI Price is lower than BB Middle, RSI Signal is lower than BBand Middle, and there has been a sell crossover
    if (InpTDICrossoverMiddleStrategy && !IsTDICrossoverMiddleTradeOpen && TDIRSIPriceCurrent < TDIBBandMiddleCurrent && TDIRSISignalCurrent < TDIBBandMiddleCurrent && TDISellCrossover)
    {
      OpenSellTrade(EntryPrice, StopLoss, TakeProfit, RiskAmount, InpStopLoss, InpRewardPercentage, InpTDICrossoverMiddleMagicNumber);
    }
  
    // If strategy is allowed, there are no open trades, RSI Price is higher than RSI Signal, there is no open wick, and the bar is bullish
    if (InpTDINoneandHAStrategy && !IsTDINoneandHATradeOpen && TDIRSIPriceCurrent > TDIRSISignalCurrent && HAOpenCurrent == HALowCurrent && HAHighCurrent > HALowCurrent)
    {
      OpenBuyTrade(EntryPrice, StopLoss, TakeProfit, RiskAmount, InpStopLoss, InpRewardPercentage, InpTDINoneandHAMagicNumber);
    }
  
    // If strategy is allowed, there are no open trades, RSI Price is lower than RSI Signal, there is no open wick, and the bar is bearish
    if (InpTDINoneandHAStrategy && !IsTDINoneandHATradeOpen && TDIRSIPriceCurrent < TDIRSISignalCurrent && HAOpenCurrent == HALowCurrent && HAHighCurrent < HALowCurrent)
    {
      OpenSellTrade(EntryPrice, StopLoss, TakeProfit, RiskAmount, InpStopLoss, InpRewardPercentage, InpTDINoneandHAMagicNumber);
    }
  
    // If strategy is allowed, there are no open trades, and there has been a buy crossover
    if (InpTDICrossoverStrategy && !IsTDICrossoverTradeOpen && TDIBuyCrossover)
    {
      OpenBuyTrade(EntryPrice, StopLoss, TakeProfit, RiskAmount, InpStopLoss, InpRewardPercentage, InpTDICrossoverMagicNumber);
    }
  
    // If strategy is allowed, there are no open trades, and there has been a sell crossover
    if (InpTDICrossoverStrategy && !IsTDICrossoverTradeOpen && TDISellCrossover)
    {
      OpenSellTrade(EntryPrice, StopLoss, TakeProfit, RiskAmount, InpStopLoss, InpRewardPercentage, InpTDICrossoverMagicNumber);
    }
