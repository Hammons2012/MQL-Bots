...
int TDITrendIndicatorHandler = 0;
int VWAPTrendIndicatorHandler = 0;
...
int OnInit()
{
  ...
  TDITrendIndicatorHandler = iCustom(Symbol(), InpTrendTimeframe, "HammonsBot/HammonsIndicator-TDI", InpTDIRsiPeriod, InpTDIBBandVol, InpTDIStandardDev, InpTDIPriceLine, InpTDIPriceType, InpTDISignalLine, InpTDISignalType);
  VWAPTrendIndicatorHandler = iCustom(Symbol(), InpTrendTimeframe, "HammonsBot/HammonsIndicator-VWAP", InpVWAPPriceType, InpVWAPEnableDaily, InpVWAPEnableWeekly, InpVWAPEnableMonthly);
...
void OnTick()
{
  ...
  //Trend indicators
  double BufferTDITrendBaseline[];
  
  ArraySetAsSeries(BufferTDITrendBaseline, true);
  
  if (CopyBuffer(TDITrendIndicatorHandler, 3, 0, 20, BufferTDITrendBaseline) < 0){Print("Copy Buffer BufferTDITrendBaseline error: " , GetLastError());}
  
  // Trend following Booleans
  if (BufferTDITrendBaseline[0] > 50)
  {
    TDIBuySignal = true;
    TDISellSignal = false;
  }
  ...
  if (InpShowExpertConfigInfo)
  {
    ...
    string TextTrendTimeframe = "Trend Timeframe: " + EnumToString(InpTrendTimeframe);
  ...
  if (CurrentBarTime != LastBarTime)
  {
    ...
    if (InpAllowCloseLogic && ((BufferTDITrendBaseline[0] < 30) || (TDISellCrossover && BufferTDIPrice[0] > 70 && BufferTDISignal[0] > 70)))
    {
      CloseAllTrades(1);
    }
    
    // If exit logic is allowed and trend TDI is above 70 or buy crossover happens below 30
    if (InpAllowCloseLogic && ((BufferTDITrendBaseline[0] > 70) || (TDIBuyCrossover && BufferTDIPrice[0] < 30 && BufferTDISignal[0] < 30)))
    {
      CloseAllTrades(2);
    }
    
    // Close Buy trades if sell trend is true
    if (InpAllowCloseLogic && TDISellSignal)
    {
      CloseAllTrades(1);
    }
    
    // Close Sell trades if buy trend is true
    if (InpAllowCloseLogic && TDIBuySignal)
    {
      CloseAllTrades(2);
    }
    
    
    // Time and day trade logic
    ...
    if (InpTDICrossoverFiftyMiddleStrategy && !IsTDICrossoverFiftyMiddleTradeOpen && TDIBuySignal && TDIBuyCrossover && BufferTDIBaseline[0] > 50 && BufferTDIBaseline[0] < 70 && BufferTDIPrice[0] < 70 && BufferTDISignal[0] < 70)
    {
    ...
    // If strategy is allowed, there are no open trades, TDI sell signal is true, there is a sell crossover, Middle BB is below 50, TD middle above 30, TDI price above 30, TDI signal above 30
    if (InpTDICrossoverFiftyMiddleStrategy && !IsTDICrossoverFiftyMiddleTradeOpen && TDISellSignal && TDISellCrossover && BufferTDIBaseline[0] < 50 && BufferTDIBaseline[0] > 30 && BufferTDIPrice[0] > 30 && BufferTDISignal[0] > 30)
    {
    ...
    if (InpTDICrossoverAboveBelowMiddleStrategy && !IsTDICrossoverAboveBelowMiddleTradeOpen && TDIBuySignal && TDIBuyCrossover && BufferTDIPrice[0] > BufferTDIBaseline[0] && BufferTDISignal[0] > BufferTDIBaseline[0] && BufferTDIBaseline[0] < 70 && BufferTDIPrice[0] < 70 && BufferTDISignal[0] < 70)
    {
    ...
    if (InpTDICrossoverAboveBelowMiddleStrategy && !IsTDICrossoverAboveBelowMiddleTradeOpen && TDISellSignal && TDISellCrossover && BufferTDIPrice[0] < BufferTDIBaseline[0] && BufferTDISignal[0] < BufferTDIBaseline[0] && BufferTDIBaseline[0] > 30 && BufferTDIPrice[0] > 30 && BufferTDISignal[0] > 30)
    {
    ...
    if (InpTDICrossoverFiftyAboveBelowMiddleHAStrategy && !IsTDICrossoverFiftyAboveBelowMiddleHATradeOpen && TDIBuySignal && TDIBuyCrossover && BufferTDIBaseline[0] > 50 && BufferTDIPrice[0] > BufferTDIBaseline[0] && BufferTDISignal[0] > BufferTDIBaseline[0] && BufferHAOpen[0] == BufferHALow[0] && BufferHAOpen[0] < BufferHAClose[0] && BufferTDIBaseline[0] < 70 && BufferTDIPrice[0] < 70 && BufferTDISignal[0] < 70)
    {
    ...
    if (InpTDICrossoverFiftyAboveBelowMiddleHAVWAPStrategy && !IsTDICrossoverFiftyAboveBelowMiddleHAVWAPTradeOpen && TDIBuySignal && TDIBuyCrossover && BufferTDIBaseline[0] > 50 && BufferTDIPrice[0] > BufferTDIBaseline[0] && BufferTDISignal[0] > BufferTDIBaseline[0] && BufferHAOpen[0] == BufferHALow[0] && BufferHAOpen[0] < BufferHAClose[0] && BufferHAOpen[0] > BufferVWAPDaily[0] && BufferTDIBaseline[0] < 70 && BufferTDIPrice[0] < 70 && BufferTDISignal[0] < 70)
    {
    ...
    // If reversal trades are allowed, TDI sell signal is true, a buy crossover happens, the TDI middle is below 30, TDI RSI price is below 30, TDI RSI signal is below 30, bar has no wick, and bar is bullish
    if (InpAllowReveralTrades && TDISellSignal && TDIBuyCrossover && BufferTDIBaseline[0] < 30 && BufferTDIPrice[0] < 30 && BufferTDISignal[0] < 30 && BufferHAOpen[0] == BufferHALow[0] && BufferHAOpen[0] < BufferHAClose[0])
    {
      // Close all trades
      CloseAllTrades(2);
    ...
    if (InpAllowReveralTrades && TDIBuySignal && TDISellCrossover && BufferTDIBaseline[0] > 70 && BufferTDIPrice[0] > 70 && BufferTDISignal[0] > 70 && BufferHAOpen[0] == BufferHAHigh[0] && BufferHAOpen[0] > BufferHAClose[0])
    {
      // Close all trades
      CloseAllTrades(1);
...
{
  // Define the position type constants
  const int FunctPositionBuy = 1;
  const int FunctPositionSell = 2;
  ...
  for (int i = 0; i < PositionsTotal(); i++)
  {
    ...
    {
      // Check the position type and close the trade if it matches
      if ((FunctOrderType == FunctPositionBuy && PositionInfo.PositionType() == POSITION_TYPE_BUY) ||
          (FunctOrderType == FunctPositionSell && PositionInfo.PositionType() == POSITION_TYPE_SELL))
      {
        // Close the position
        ...
        {
          Print("Failed to close position. Last error: ", GetLastError());
        }
      }
    }
  }
